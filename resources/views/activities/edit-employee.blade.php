<x-admin-layout>
    <x-slot name="header">
        Edit Activity/Travel
    </x-slot>
    <div class="min-h-screen bg-gray-50">
    <div class="w-full max-w-3xl mx-auto px-2 sm:px-4 lg:px-6 py-6 sm:py-8">
        <div class="mb-6">
            <a href="{{ route('activities.index') }}" class="text-blue-600 hover:text-blue-900 inline-flex items-center">
        ...
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('activities.update', $activity) }}" method="POST" class="p-4 sm:p-6 space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <input type="hidden" name="employee_id" value="{{ $activity->employee_id }}">

                <div class="rounded-md bg-blue-50 p-3 text-sm text-blue-800">
                    Employee: <span class="font-semibold">{{ $activity->employee?->getFullName() ?? 'N/A' }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $activity->end_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text" id="activity_type" name="activity_type" value="{{ old('activity_type', $activity->activity_type) }}" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="mt-1 block w-full px-4 py-3 text-base border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $activity->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="url" id="memorandum_link" name="memorandum_link" value="{{ old('memorandum_link', $activity->memorandum_link) }}" placeholder="https://drive.google.com/file/d/..." 
                        ...
                        @if (!empty($activity->memorandum_link))
                            <p class="text-xs text-gray-500 mt-1">Current: <a href="{{ $activity->memorandum_link }}" target="_blank" class="text-blue-600 underline">Open PDF</a></p>
                        @endif
                    ...
                    <div>
                        ...
                        @if (!empty($activity->certificate_attachment))
                            <p class="text-xs text-gray-500 mt-1">Current: <a href="{{ asset('storage/' . $activity->certificate_attachment) }}" target="_blank" class="text-green-600 underline">View Attachment</a></p>
                        @endif
                        <p class="text-xs text-gray-500 mt-1">CA/Travel. PDF, JPG, or PNG file. Optional. Upload to replace.</p>
                    </div>
                    ...
                    <div>
                        ...
                        @if (!empty($activity->att_attachment))
                            <p class="text-xs text-gray-500 mt-1">Current: <a href="{{ asset('storage/' . $activity->att_attachment) }}" target="_blank" class="text-yellow-600 underline">View Attachment</a></p>
                        @endif
                        <p class="text-xs text-gray-500 mt-1">PDF, JPG, or PNG file. Optional. Upload to replace.</p>
                    </div>
                </div>
