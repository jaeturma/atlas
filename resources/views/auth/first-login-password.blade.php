    ...
    <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        ...
        <div class="min-h-screen flex items-center justify-center bg-white py-10 md:py-0">
            <div class="w-full max-w-md px-6 sm:px-8">
                ...
                <form method="POST" action="{{ route('first-login.password.post') }}" class="mt-6 space-y-4">
                    @csrf

                    <div>
                        <x-input-label for="password" :value="__('Create Password')" />
                        <x-text-input id="password" class="block mt-1 w-full text-xl sm:text-2xl py-4" type="password" name="password" required />
                        <x-input-error :messages="$errors->get('password')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="password_confirmation" :value="__('Confirm Password')" />
                        <x-text-input id="password_confirmation" class="block mt-1 w-full text-xl sm:text-2xl py-4" type="password" name="password_confirmation" required />
                    </div>
