<x-guest-layout fullScreen="true">
    <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        ...
        <div class="min-h-screen flex items-center justify-center bg-white py-10 md:py-0">
            <div class="w-full max-w-md px-6 sm:px-8">
                ...
                <form method="POST" action="{{ route('first-login.verify.post') }}" class="mt-6 space-y-4">
                    @csrf

                    <div>
                        <x-input-label for="otp" :value="__('Enter OTP')" />
                        <x-text-input id="otp" class="block mt-1 w-full text-xl sm:text-2xl py-4" type="text" name="otp" required autofocus maxlength="6" />
                ...
                <div class="mt-6 text-sm text-gray-600" id="otp-help" data-sent-at="{{ $sentAt }}">
                    OTP expires in 10 minutes. You can request a new OTP after 3 minutes.
                </div>

                <form method="POST" action="{{ route('first-login.send') }}" class="mt-3" id="resend-form">
                    @csrf
                    <input type="hidden" name="email" value="{{ session('first_login_email') }}">
                    <input type="hidden" name="sent_at" value="{{ $sentAt }}">
                    <input type="hidden" name="sent_sig" value="{{ $sentSig }}">
                    <x-primary-button id="resend-btn" disabled class="w-full justify-center py-4 text-lg sm:text-xl">
                        {{ __('Resend OTP') }}
                    </x-primary-button>
                </form>

                <div class="mt-2 text-xs text-gray-500" id="resend-countdown"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const sentAt = parseInt(help?.dataset?.sentAt || '0', 10);
            const cooldownMs = 3 * 60 * 1000;

            if (!sentAt || !resendBtn || !countdownEl) {
                return;
            }

            const tick = () => {
                const now = Date.now();
                const remaining = (sentAt * 1000 + cooldownMs) - now;

                if (remaining <= 0) {
                    resendBtn.disabled = false;
                    countdownEl.textContent = 'You can request a new OTP now.';
                    return;
                }
                ...
                countdownEl.textContent = `Resend available in ${minutes}:${secs}`;
