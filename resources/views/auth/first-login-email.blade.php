    ...
    <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        ...
        <div class="min-h-screen flex items-center justify-center bg-white py-10 md:py-0">
            <div class="w-full max-w-md px-6 sm:px-8">
                ...
                <form method="POST" action="{{ route('first-login.send') }}" class="mt-6 space-y-4">
                    @csrf

                    <div>
                        <x-input-label for="email" :value="__('Work Email')" />
                        <x-text-input id="email" class="block mt-1 w-full text-xl sm:text-2xl py-4" type="email" name="email" :value="old('email')" required autofocus />
